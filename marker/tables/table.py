...
def get_table_boxes(pages: List[Page], doc: PdfDocument, fname, tables_path, save_table):
    table_imgs = []
    table_counts = []
    table_bboxes = []
    img_sizes = []
    ...
    for page_idx, page in enumerate(pages):
        ...
        for bb in bbox:
            ...
            table_img = highres_img.crop(highres_bb)  # Cropping the image around each table bounding box

            image_path = os.path.join(tables_path, f'page_{page_idx + 1}_table_{len(table_imgs) + 1}.png')
            if(save_table == 2 or save_table == 3):
                table_img.save(image_path, "PNG")  # Save the image
            page_table_imgs.append(table_img)
...
def format_tables(images, pages: List[Page], doc: PdfDocument, fname: str, detection_model, table_rec_model, ocr_model, save_table, tables_path):
    ...
    tqdm.disable = True
    table_imgs, table_boxes, table_counts, table_text_lines, img_sizes = get_table_boxes(pages, doc, fname,tables_path, save_table)
    ...
    for page_idx, page in enumerate(pages):
        ...
        for table_idx, table_box in enumerate(page_table_boxes):
            ...
            if(save_table == 1):
                ...
                markdown = table_md[table_count]
                path = os.path.join('.\\tables', f'page_{page_idx + 1}_table_{table_idx + 1}.png')
                markdown+=f'\n ![Benchmark overall]({path})'
            else:
                markdown = ''
    ...
    if(save_table == 1 or save_table == 3):
        table_md = [formatter("markdown", cell)[0] for cell in cells]
    ...
    for page_idx, page in enumerate(pages):
        ...
        for table_idx, table_box in enumerate(page_table_boxes):
            ...
            if(save_table == 1):
                ...
                markdown = table_md[table_count]
                path = os.path.join(tables_path, f'page_{page_idx + 1}_table_{len(table_imgs) + 1}.png')
                markdown+=f'\n ![Benchmark overall]({path})'
