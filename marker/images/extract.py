...
from marker.schema.block import Line, Span, find_insert_block
...
def find_image_blocks(page, min_image_height, min_image_width):
    ...
    if (
        min_image_width < 0
        or min_image_width > 1
        or min_image_height < 0
        or min_image_height > 1
    ):
    ...
    #     if (region[2] - region[0] >= min_width and region[3] - region[1] >= min_height)
    # ]
    # ic(image_regions)
    insert_points = {}
